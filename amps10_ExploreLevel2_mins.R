...
# ind_ct <- nearZeroVar(set10_CT[,21:ncol(set10_CT)], saveMetrics = TRUE)
# ind_jhb <- nearZeroVar(set10_JHB[,21:ncol(set10_JHB)], saveMetrics = TRUE)
# 
# good_ct <- set10_CT[,21:ncol(set10_CT)][,!ind_ct$zeroVar]
# good_jhb <- set10_JHB[,21:ncol(set10_JHB)][,!ind_jhb$nzv]
# 
# catSet10_CT <- data.frame(cbind(set10_CT[,1:20], good_ct))
# catSet10_JHB <- data.frame(cbind(set10_JHB[,1:20], good_jhb))
# 
# saveRDS(catSet10_CT, "catSet10_CT.rds")
# saveRDS(catSet10_JHB, "catSet10_JHB.rds")
# 
# nuSet10_CT <- data.frame(cbind(set10_CT[,1:20], good_ct))
# nuSet10_JHB <- data.frame(cbind(set10_JHB[,1:20], good_jhb))

#setting the ordered variables as scaled numerical:
set10_nat$age <- scale(as.numeric(set10_nat$age))
set10_nat$edu <- scale(as.numeric(set10_nat$edu))
set10_nat$hh_inc <- scale(as.numeric(set10_nat$hh_inc))
set10_nat$lsm <- scale(as.numeric(set10_nat$lsm))
...
set10_nat$sex <- factor(set10_nat$sex,
                        levels = c(1,2),
                        labels = c("male", "female"))
set10_nat$race <- factor(set10_nat$race,
                         levels = c(1,2,3,4),
                         labels = c("black", "coloured", "indian", "white"))
set10_nat$lifestages <- factor(set10_nat$lifestages,
...
set10_nat$mar_status <- factor(set10_nat$mar_status,
                               levels = c(1,2,3,4,5),
                               labels = c("single", "married or living together", "widowed", "divorced", "separated"))
set10_nat$lifestyle <- factor(set10_nat$lifestyle,
...
set10_nat$attitudes <- factor(set10_nat$attitudes,
...
set.seed(56)
pca_10_nat <- PCA(set10_nat,
                  quanti.sup = c(1,3,4,6),
                  quali.sup = c(2,5),
                  ncp = npc,
                  graph = FALSE)
saveRDS(pca_10_nat, "pca_10_nat.rds")


# try pa method of factor analysis with oblimin rotation allowed....to try and get better estimation
library(psych)
set.seed(123)
fact_10 <- fa(set10_nat[7:ncol(set10_nat)], nfactors = 6, fm = "pa") # default rotation oblimin, so does allow correlation between factors
...
fact_10_scores_model08 <- predict(fact_08, data = set10_min[,7:ncol(set08_min)])

# rather try print as table for importing:
capture.output(print(fact_10$loadings,digits=2,all=FALSE,cut=0.1,sort=TRUE,short=TRUE,lower=TRUE,signif=NULL), file = "/Users/HansPeter/Dropbox/Statistics/UCTDataScience/Thesis/amps_2008/loadings.csv", append = TRUE)


# # try FactoInvestigate
# library(FactoInvestigate)
# Investigate(pca_10_nat, "factoInvestigate_10_nat.Rmd", c("pdf_document"))

# contributions plots
jpeg("contributions10_nat_1n2.jpeg")
fviz_pca_var(pca_10_nat,
             axes = c(1,2),
             col.var="contrib",
...
jpeg("contributions10_nat_3n4.jpeg")
fviz_pca_var(pca_10_nat,
             axes = c(3,4),
             col.var="contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
...
jpeg("contributions10_nat_5n6.jpeg")
fviz_pca_var(pca_10_nat,
             axes = c(5,6),
             col.var="contrib",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
...
# for National
vehicle_nat <- rownames(pca_10_nat$var$cor)
corr_nat <- as.data.frame(pca_10_nat$var$cor)
contrib_nat <- as.data.frame(pca_10_nat$var$contrib)
cos2_nat <- as.data.frame(pca_10_nat$var$cos2)
...
dimdesc_10_nat <- dimdesc(pca_10_nat, c(1:npc), proba = 1)

# categorical supplementaries per dimension ... need to explain and interpret "Estimate"
cat_coord_10_nat <- list()
for(i in 1:npc) {
        temp1 <- dimdesc_10_nat[[i]]$category[order(dimdesc_10_nat[[i]]$category[,1], decreasing = TRUE),]
        temp2 <- temp1[c(1:5, (nrow(temp1) - 4): nrow(temp1)),1]
        cat_coord_10_nat[[i]] <- data.frame(Est = round(temp2, 2))
        # write.table(cat_coord_10_nat[[i]], file = paste0("cat_dim", i, "_nat.csv")) # if needed
        
}

# getting tables 
# Dim 1 CT
tab_nat_cat1 <- tableGrob(cat_coord_10_nat[[1]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 2 CT
tab_nat_cat2 <- tableGrob(cat_coord_10_nat[[2]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 3 CT
tab_nat_cat3 <- tableGrob(cat_coord_10_nat[[3]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 4 CT
tab_nat_cat4 <- tableGrob(cat_coord_10_nat[[4]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 5 CT
tab_nat_cat5 <- tableGrob(cat_coord_10_nat[[5]], theme = ttheme_minimal(base_size = 12)) # table
...
# Dim 6 CT
tab_nat_cat6 <- tableGrob(cat_coord_10_nat[[6]], theme = ttheme_minimal(base_size = 12)) # table
...
cont_corrs_10_nat <- matrix(0, nrow = 4, ncol = 0)
for(i in 1:npc) {
        temp1 <- as.matrix(dimdesc_10_nat[[i]]$quanti)
        temp2 <- temp1[which(rownames(temp1) %in% c("age", "edu", "hh_inc", "lsm")),]
        cont_corrs_10_nat <- as.data.frame(round(cbind(cont_corrs_10_nat, temp2), 2))
}
names(cont_corrs_10_nat) <- c("Dim1", "pVal","Dim2", "pVal", "Dim3", "pVal", "Dim4", "pVal", "Dim5", "pVal", "Dim6", "pVal" )

# print to file for graphic:

tab_nat_cont <- tableGrob(cont_corrs_10_nat, theme = ttheme_minimal(base_size = 8)) # table
